...
export const DATASETS: DatasetSpec[] = [
  ...
  },
  {
    ...
    source:      'WWF / HydroSHEDS HydroBASINS v1c',
    toFile: (r) => ({
      country_name: String(r.country),
      country_iso3: iso3For(String(r.country)),
      r2_key:       String(r.r2_key),
  ...
  },
  {
    ...
    description: 'Natural Earth 1:10m significant roads (Freeway, Primary, Secondary, Other Paved) per African country.',
    ...
    toFile: (r) => ({
      ...
      file_size_mb: Number(r.file_size_mb ?? 0),
      file_format:  String(r.file_format ?? 'GeoPackage'),
      source:       String(r.source ?? 'Natural Earth 1:10m Roads'),
      meta: {
        feature_count: r.feature_count,
        total_km:      r.total_km,
        epsg:          r.epsg,
  ...
  {
    id:          'soil',
    name:        'Soil classification (WRB)',
    description: 'ISRIC SoilGrids v2.0 WRB most-probable soil classification at 250m resolution per African country.',
    ...
    toFile: (r) => ({
      ...
      file_size_mb: Number(r.file_size_mb ?? 0),
      file_format:  String(r.file_format ?? 'GeoTIFF'),
      source:       String(r.source ?? 'ISRIC SoilGrids v2.0'),
      meta: {
        resolution_m:   r.resolution_m,
        classification: 'WRB Most-Probable',
        epsg:           r.epsg,
      },
    }),
  },
  {
    ...
    description: 'National parks, game reserves, conservancies, forest reserves and other designated conservation zones per African country, extracted from OpenStreetMap (boundary=protected_area, leisure=nature_reserve).',
    category:    'environment',
    ...
    toFile: (r) => ({
      ...
      file_size_mb: Number(r.file_size_mb ?? 0),
      file_format:  String(r.file_format ?? 'Shapefile (ZIP)'),
      source:       String(r.source ?? 'OpenStreetMap contributors · ODbL'),
      meta: {
        feature_count:       r.feature_count,
